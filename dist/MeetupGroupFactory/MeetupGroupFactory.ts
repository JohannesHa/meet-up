...
class CreateGroup extends EthereumEvent {
  get params(): CreateGroupParams {
...
class MeetupGroupFactory extends SmartContract {
  static bind(address: Address, blockHash: H256): MeetupGroupFactory {
    return new MeetupGroupFactory("MeetupGroupFactory", address, blockHash);
  }

  subdomainFactory(): Address {
    let result = super.call("subdomainFactory", []);
  ...
  groups(param0: U256): Address {
    let result = super.call("groups", [EthereumValue.fromU256(param0)]);
    return result[0].toAddress();
  }
}
